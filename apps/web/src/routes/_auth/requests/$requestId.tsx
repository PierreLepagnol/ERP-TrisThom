...
import { activeRequestStatuses, commercialStatusValues, getAllowedRequestStatuses, normalizeRequestStatus, requestStatusConfig, requestStatusValues, type RequestStatus } from "@/domain/request-status";
...
function RequestDetailPage() {
  ...
  const hasPreparation = ["accepte", "termine"].includes(normalizeRequestStatus(request.status));
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="flex flex-wrap items-start justify-between gap-4">
        <div><h1 className="font-serif text-4xl font-bold">{request.contactName}</h1><p className="mt-2 text-sm text-stone-600">{request.eventType || "Événement à préciser"} · <strong>{request.eventDate ? dateFormat.format(request.eventDate) : "Date à préciser"}</strong>{request.guestCount ? ` · ${request.guestCount} personnes` : ""}</p><p className="mt-1 text-sm text-stone-500">{request.eventAddress || request.venue || "—"}</p><div className="mt-2"><RequestSourceBadge source={request.source} /></div></div>
        <div className="flex items-center gap-2"><label className="text-sm font-bold">Statut <select value={normalizeRequestStatus(request.status)} onChange={(event) => void changeStatus(event.target.value as RequestStatus)} className="input ml-1 inline-block w-auto">{commercialStatusValues.map((value) => <option key={value} value={value}>{requestStatusConfig[value].label}</option>)}</select></label><details className="relative"><summary aria-label="Autres actions" className="list-none cursor-pointer rounded-md border border-stone-200 bg-white p-2 text-stone-700"><MoreHorizontal className="size-5" /></summary><div className="absolute right-0 z-20 mt-2 grid min-w-48 gap-1 rounded-lg border border-stone-200 bg-white p-2 shadow-lg"><button onClick={() => setEditing(true)} className="rounded px-3 py-2 text-left text-sm font-semibold hover:bg-stone-50">Modifier</button><button onClick={() => setDeleteDialog(true)} className="rounded px-3 py-2 text-left text-sm font-semibold text-red-700 hover:bg-red-50">Supprimer</button></div></details></div>
      </section>
      ...
      {sameDayOpen ? <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4"><section className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl"><div className="flex justify-between gap-3"><h2 className="font-serif text-2xl font-bold">Dossiers du même jour</h2><button onClick={() => setSameDayOpen(false)} className="font-bold">Fermer</button></div><div className="mt-4 divide-y divide-stone-100">{sameDayRequests.map((item) => <button key={item._id} onClick={() => void navigate({ to: "/requests/$requestId", params: { requestId: item._id } })} className="block w-full py-3 text-left hover:text-[#8b1629]"><strong>{item.contactName}</strong> · {requestStatusConfig[item.status].label}{item.guestCount ? ` · ${item.guestCount} pers.` : ""}</button>)}</div></section></div> : null}
    </div>
  );
  return (
